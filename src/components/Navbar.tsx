...
const Navbar: React.FC = () => {
	...
	const { content, contentList, open } = useAppSelector(navbarSelector)

	return (
		<nav
			onClick={() => dispatch(setOpen(!open))}
			onMouseEnter={() => dispatch(setOpen(true))}
			onMouseLeave={() => dispatch(setOpen(false))}
			className='nav'
		>
			<div className='nav__lable'>
			...
			{open && (
				<ul className='tabs'>
					{contentList.map((obj, i) => (
						<li
							key={i}
							onClick={() => dispatch(setContent(obj))}
							className='tab'
						>
							<Link to={obj.property}>{obj.name}</Link>
						</li>
					))}
				</ul>
			)}
